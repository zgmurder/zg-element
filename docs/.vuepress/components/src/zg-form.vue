<template>
  <el-form ref="el-form" v-bind="$attrs" :model="formData" v-on="$listeners">
    <el-form-item v-for="(item, index) in schema" v-show="!item.hidden" :key="index" :prop="item.vModel" v-bind="handleFormItemBind(item)">
...
export default {
  ...
  methods: {
    handleFormItemBind(item) {
      // eslint-disable-next-line no-unused-vars
      const { config, resetField, clearValidate, ...data } = item
      return  data
    },
    ...
    handleLabelOrValue(item, subItem, index, type) {
      if (item.config && item.config[`handle${type}`]) return item.config.handleLabel(subItem)
      else return subItem
    },
